...
import { COMPANY, REDRESS_CONTACTS } from "@/lib/consent-policy/company";
 ...
 * lib/consent-policy/markdown.ts의 buildPolicySections와 같은 조항 구성·순서를 따른다.
 */
function buildPolicySections(doc: PolicyDocument): PolicySection[] {
  const sections: PolicySection[] = [
    {
      ...
      render: () => (
        <>
          <p className="mb-3">
            회사는 다음의 개인정보 항목을 개인정보 보호법 제15조 제1항 제1호, 제22조 제1항 제1호에 따라
            정보주체의 동의를 받아 처리하고 있습니다.
          </p>
          <Table
          ...
          {doc.lawRows.length > 0 ? (
            <>
              <h5 className="mb-2 text-sm font-bold">1-1) 관계 법령에 따른 보유 및 이용 기간</h5>
              <p className="mb-2">회사는 관계 법령의 규정에 해당하는 경우 아래와 같이 개인정보를 보존합니다.</p>
              <Table
      ...
      ? [
          {
            ...
            render: () => (
              <>
                <p className="mb-3">
                  회사는 만 14세 미만 아동의 개인정보를 처리하기 위하여 동의가 필요한 경우, 법정대리인의 동의를
                  받은 후에만 처리합니다.
                </p>
                <p className="mb-3">
                  법정대리인의 동의를 받을 때에는 법정대리인 확인에 필요한 최소한의 정보만 수집하며, 그 목적
                  외로 이용하지 않습니다.
                </p>
                <p className="mb-3">
                  법정대리인은 아동의 개인정보에 대한 열람, 정정·삭제, 처리정지 및 동의 철회를 요구할 수 있고,
                  아동 본인이 요청하는 경우에도 법정대리인의 동의 여부를 확인한 후 처리합니다.
                </p>
              </>
            ),
          },
    ...
    },
    {
      ...
      render: () => (
        <p className="mb-3">
          회사는 개인정보 보호법 제15조 제3항 및 제17조 제4항에 따라, 당초 수집 목적과의 관련성, 처리 관행에
          비춘 예측 가능성, 정보주체의 이익 침해 여부, 가명처리 또는 암호화 등 안전성 확보 조치 여부를 고려한
          후에만 정보주체의 동의 없이 개인정보를 추가적으로 이용·제공합니다.
        </p>
      ),
    },
    {
      title: "개인정보의 국외이전",
      render: () => <p className="mb-3">회사는 이용자의 정보를 국외로 이전하지 않습니다.</p>,
    },
    {
      title: "개인정보 파기 절차 및 방법에 관한 사항",
      render: () => (
        <p className="mb-3">
          회사는 이용자의 개인정보를 수집 및 이용목적이 달성되면 지체 없이 파기합니다. 종이에 출력된 개인정보는
          분쇄기로 분쇄하거나 소각을 통하여 파기하고, 전자적 파일형태로 저장된 개인정보는 기록을 재생할 수 없는
          기술적 방법을 사용하여 삭제합니다.
        </p>
      ),
    },
    {
      ...
      render: () => (
        <p className="mb-3">
          회사는 개인정보처리시스템의 접근 권한 관리와 접근 통제, 접속 기록 보관, 개인정보의 암호화 등 기술적
          조치와, 내부관리계획 수립·시행 및 취급 직원 정기 교육 등 관리적 조치, 전산실과 자료 보관실의
          출입통제 등 물리적 조치를 시행하고 있습니다.
        </p>
      ),
    },
    {
      ...
      render: () => (
        <p className="mb-3">
          이용자 및 법정대리인은 언제든지 자신의 개인정보를 열람하거나 정정할 수 있고 삭제 및 처리 정지를
          요구할 수 있으며, 수집·이용·제공에 대한 동의를 철회할 수 있습니다. 아래 개인정보 보호책임자 조항의
          연락처로 서면 또는 전자우편으로 요청하시면 지체 없이 조치하겠습니다.
        </p>
      ),
    },
    {
      ...
      render: () =>
        doc.usesCookies ? (
          <>
            <p className="mb-3">
              회사는 서비스를 운영하는 과정에서 이용자의 서비스 이용에 필요한 항목을 자동으로 수집하기
              위하여, 이용자의 정보를 수시로 저장하고 찾아내는 &apos;쿠키(cookie)&apos; 등 개인정보를 자동으로
              수집하는 장치를 설치·운용합니다.
            </p>
            <p className="mb-3">
              쿠키란 웹사이트를 운영하는 데 이용되는 서버가 이용자의 브라우저에 보내는 아주 작은 텍스트
              파일로서 이용자의 컴퓨터 하드디스크에 저장됩니다.
            </p>
            <p className="mb-3">
              이용자는 웹 브라우저의 옵션을 설정함으로써 모든 쿠키를 허용하거나 쿠키를 저장할 때마다 확인을
              거치거나, 모든 쿠키의 저장을 거부할 수 있습니다. 단, 쿠키 설치를 거부할 경우 서비스 제공에
              어려움이 있을 수 있습니다.
            </p>
            <p className="mb-1">웹 브라우저에서 쿠키를 허용하거나 차단하는 방법은 다음과 같습니다.</p>
            <p className="mb-3">
              - 크롬(Chrome): 웹 브라우저 설정 &gt; 개인정보 보호 및 보안 &gt; 인터넷 사용 기록 삭제
              <br />- 엣지(Edge): 웹 브라우저 설정 &gt; 쿠키 및 사이트 권한 &gt; 쿠키 및 사이트 데이터 관리 및
              삭제
            </p>
          </>
        ) : (
    ...
    },
    {
      ...
      render: () => (
        <p className="mb-3">
          {REDRESS_CONTACTS.map((contact) => (
            <span key={contact.name} className="block">
              {contact.name}: {contact.phone} ({contact.url})
            </span>
          ))}
        </p>
      ),
  ...
  if (doc.usesLocation) {
    sections.push({
      ...
      render: () => (
        <>
          <p className="mb-3">
            회사는 위치기반서비스의 제공을 위하여 이용자의 개인위치정보를 수집·이용하며, 이 경우
            「위치정보의 보호 및 이용 등에 관한 법률」이 개인정보 보호법과 함께 적용됩니다.
          </p>
          <p className="mb-3">
            회사는 개인위치정보를 이용자가 지정한 제3자에게 제공하는 경우에는 개인위치정보를 수집한 해당
            통신단말장치로 매회 이용자에게 제공받는 자, 제공 일시 및 제공 목적을 즉시 통보합니다.
          </p>
          <p className="mb-3">
            이용자는 언제든지 개인위치정보의 수집·이용·제공에 대한 동의의 전부 또는 일부를 철회할 수 있고,
            일시적으로 위치정보의 수집·이용·제공을 중지할 것을 요구할 수 있습니다. 이 경우 회사는 정당한
            사유 없이 요구를 거절하지 않으며 이를 위한 기술적 수단을 갖추고 있습니다.
          </p>
          <p className="mb-3">
            이용자는 개인위치정보의 수집·이용·제공사실 확인자료의 열람 또는 고지를 요구할 수 있으며, 자신의
            개인위치정보와 관련하여 오류가 있는 경우 정정을 요구할 수 있습니다.
          </p>
        </>
      ),
    });
  ...
  sections.push({
    ...
    render: () => <p>본 개인정보 처리방침은 {doc.effectiveDateLabel}부터 시행합니다.</p>,
  });
  ...
  return (
    <div className="rounded-xl border bg-card p-10 text-sm leading-[1.8]">
      ...
      <p className="mb-3">
        ...
        <span className="font-semibold text-primary">{doc.serviceName || "(서비스명 미입력)"}</span> 서비스(이하
